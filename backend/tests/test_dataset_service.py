    ...
    df = pd.DataFrame(
        {
            ...
            "col_cat": pd.Series(
                ["apple", "banana", "apple", "banana"], dtype="category"
            ),
    ...
    df = pd.DataFrame(
        {
            "col_test": [
                ...
                None,
                2,
            ],  # 4 rows, 1 null (25%), unique non-nulls: [1, 2] (count = 2)
        }
    )
...
async def test_500mb_warning(db_session: AsyncSession, sample_project: Project) -> None:
    ...
    with patch("pandas.read_csv", return_value=mock_df), patch(
        "pandas.DataFrame.to_parquet"
    ), patch(
        "openneural_backend.services.dataset_service._set_snapshot_file_permissions"
    ), patch(
        "openneural_backend.services.dataset_service.logger"
    ) as mock_logger:
...
async def test_parquet_conversion(
    ...
    parquet_path = (
        Settings.get().data_dir / "snapshots" / snapshot_dict["id"] / "data.parquet"
    )
...
@pytest.mark.anyio
async def test_get_snapshot_exceptions(
