...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime, nullable=False, server_default=sa.func.now()),
        sa.Column("updated_at", sa.DateTime, nullable=False, server_default=sa.func.now()),
        sa.CheckConstraint("task_type IN ('classification', 'regression')", name="ck_project_task_type"),
    )
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("project_id", "version_label", name="uq_snapshot_project_version"),
    )
